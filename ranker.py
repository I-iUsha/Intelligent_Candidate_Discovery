...
def generate_reasoning(result: dict, candidate: dict) -> str:
    """
    Generate specific, honest reasoning for a candidate's ranking.
    References actual profile facts — no hallucination.
    """
    ...
    top_skills = sorted(skills, key=lambda s: skill_quality_score(s), reverse=True)[:4]
    top_skill_names = [s["name"] for s in top_skills]

    title = m["title"]
    yoe = m["yoe"]
    location = m["location"]
    notice = m["notice_days"]
    response_rate = m["response_rate"]
    last_active = m["last_active"]
    open_to_work = m["open_to_work"]

    # Build reasoning from actual signals
    strengths = []
    concerns = []

    # Skills
    if p["skill_fit"] >= 0.70:
        strengths.append(f"strong retrieval/embedding skillset ({', '.join(top_skill_names[:3])})")
    elif p["skill_fit"] >= 0.45:
        strengths.append(f"relevant skills ({', '.join(top_skill_names[:2])})")

    # Career depth
    if p["ai_career_depth"] >= 0.70:
        strengths.append(f"{yoe:.1f}yr career deeply rooted in AI/ML engineering")
    elif p["ai_career_depth"] >= 0.40:
        strengths.append(f"partial AI/ML career depth over {yoe:.1f}yr")
    else:
        concerns.append(f"limited AI/ML career evidence despite {yoe:.1f}yr experience")

    # Location
    if p["loc_fit"] >= 0.90:
        strengths.append(f"based in {location} (JD-preferred location)")
    elif p["loc_fit"] >= 0.75:
        strengths.append(f"India-based ({location})")
    elif m["willing_to_relocate"]:
        strengths.append(f"based in {location}, willing to relocate to India")
    else:
        concerns.append(f"based outside India ({location}), not willing to relocate")

    # Availability
    if p["availability"] >= 0.80:
        if open_to_work:
            strengths.append("actively open to work with strong platform engagement")
    elif p["availability"] <= 0.50:
        if notice > 90:
            concerns.append(f"{notice}-day notice period")
        if response_rate < 0.3:
            concerns.append(f"low recruiter response rate ({response_rate:.0%})")

    # Title corroboration — only flag if title is genuinely non-engineering
    genuine_non_eng = ["marketing", "sales", "accountant", "hr", "customer support",
                       "operations manager", "civil engineer", "mechanical engineer",
                       "content writer", "graphic designer"]
    if p["title_corroboration"] < 0.40 and any(t in title.lower() for t in genuine_non_eng):
        concerns.append(f"current title ({title}) doesn't align with AI engineering")

    # Compose final string (1-2 sentences)
    strength_str = "; ".join(strengths[:2]) if strengths else f"{title} with {yoe:.1f}yr exp"
    concern_str = (", but " + "; ".join(concerns[:1])) if concerns else ""

    reasoning = f"{title}, {yoe:.1f}yr - {strength_str}{concern_str}."
    # Cap cleanly - never cut mid-phrase
    if len(reasoning) > 200:
        truncated = reasoning[:197]
        last_semi = truncated.rfind(';')
        last_comma = truncated.rfind(',')
        cut = max(last_semi, last_comma)
        if cut > 120:
            reasoning = truncated[:cut].rstrip(' ,;') + "."
        else:
            last_space = truncated.rfind(' ')
            reasoning = truncated[:last_space].rstrip(' ,;') + "."
